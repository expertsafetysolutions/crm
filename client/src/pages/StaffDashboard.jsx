...
export default function StaffDashboard() {
        ...
        <div className="flex flex-col gap-2 p-3">
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            >
              ...
              {filterStatus === 'ALL' && isCompleted && (
                <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-800">
                  Closed / Completed
                </span>
              )}
              {filterStatus === 'ALL' && task.Status === 'Removal Requested' && (
                <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-amber-100 text-amber-900 border border-amber-300">
                  ⏳ Removal Requested
                </span>
              )}
              {filterStatus === 'ALL' && !isCompleted && task.Status !== 'Removal Requested' && (
  ...
  return (
    <div className="max-w-5xl mx-auto px-3 sm:px-6 py-4 sm:py-6 space-y-4 sm:space-y-6">
      ...
      {/* REMARKS & INTERACTION HISTORY MODAL — full-screen with back button, so the History list gets maximum screen space */}
